    ...
    if (isset($_POST['action'])) {
        if ($_POST['action'] == 'add' && isset($_POST['name'])) {
            ...
            try {
                ...
                if ($isDefault) {
                    $stmt = $pdo->prepare("UPDATE content_production_statuses SET is_default = 0 WHERE company_id = ?");
                    $stmt->execute([$companyId]);
                }
                
                $stmt = $pdo->prepare("INSERT INTO content_production_statuses (company_id, name, is_default) VALUES (?, ?, ?)");
                $stmt->execute([$companyId, $name, $isDefault]);
                
                setSuccess('وضعیت تولید جدید با موفقیت اضافه شد.');
                redirect("content_production_statuses.php?company=$companyId");
            } catch (PDOException $e) {
                setError('خطا در افزودن وضعیت تولید.');
            }
            ...
            try {
                ...
                $stmt = $pdo->prepare("SELECT is_system FROM content_production_statuses WHERE id = ? AND company_id = ?");
                ...
                $stmt = $pdo->prepare("UPDATE content_production_statuses SET name = ?, is_default = ? WHERE id = ? AND company_id = ?");
                $stmt->execute([$name, $isDefault, $id, $companyId]);
                
                setSuccess('وضعیت تولید با موفقیت ویرایش شد.');
                redirect("content_production_statuses.php?company=$companyId");
            } catch (PDOException $e) {
                setError('خطا در ویرایش وضعیت تولید.');
            }
            ...
            try {
                ...
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM contents WHERE production_status_id = ?");
                $stmt->execute([$id]);
                ...
                $stmt = $pdo->prepare("DELETE FROM content_production_statuses WHERE id = ? AND company_id = ? AND is_system = 0");
                $stmt->execute([$id, $companyId]);
                
                setSuccess('وضعیت تولید با موفقیت حذف شد.');
                redirect("content_production_statuses.php?company=$companyId");
            } catch (PDOException $e) {
                setError('خطا در حذف وضعیت تولید.');
            }
        }
    }
}
...
$stmt = $pdo->prepare("SELECT * FROM content_production_statuses WHERE company_id = ? ORDER BY is_system DESC, name");
$stmt->execute([$companyId]);
